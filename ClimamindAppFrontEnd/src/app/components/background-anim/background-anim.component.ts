import {
  Component, Input, OnChanges, SimpleChanges,
  ViewChild, ElementRef, AfterViewInit, OnDestroy,
  NgZone, ChangeDetectionStrategy
} from '@angular/core';
import { CommonModule } from '@angular/common';
import { WeatherCondition } from '../../core/models/weather.model';

@Component({
  selector: 'app-background-anim',
  standalone: true,
  imports: [CommonModule],
  changeDetection: ChangeDetectionStrategy.OnPush,
  templateUrl: './background-anim.component.html',
  styleUrls: ['./background-anim.component.scss']
})
export class BackgroundAnimComponent implements AfterViewInit, OnChanges, OnDestroy {
  @Input() condition: WeatherCondition = 'clear';
  @ViewChild('canvas') canvasRef!: ElementRef<HTMLCanvasElement>;

  rays = Array.from({ length: 12 }, (_, i) => i * 30);
  showSun = false;
  showClouds = false;
  darkClouds = false;
  lightningFlash = false;

  private ctx!: CanvasRenderingContext2D;
  private animId!: number;
  private drops: Drop[] = [];
  private flakes: Flake[] = [];
  private mistLayers: MistLayer[] = [];
  private lightningTimer = 0;
  private W = 0;
  private H = 0;

  constructor(private zone: NgZone) {}

  ngAfterViewInit() {
    this.ctx = this.canvasRef.nativeElement.getContext('2d')!;
    this.resize();
    window.addEventListener('resize', () => this.resize());
    this.applyCondition();
    // Run animation OUTSIDE Angular zone for performance
    this.zone.runOutsideAngular(() => this.animate());
  }

  ngOnChanges(changes: SimpleChanges) {
    if (changes['condition'] && this.ctx) {
      this.applyCondition();
    }
  }

  ngOnDestroy() {
    cancelAnimationFrame(this.animId);
    window.removeEventListener('resize', () => this.resize());
  }

  private resize() {
    const c = this.canvasRef.nativeElement;
    c.width = this.W = window.innerWidth;
    c.height = this.H = window.innerHeight;
    this.applyCondition();
  }

  private applyCondition() {
    this.drops = [];
    this.flakes = [];
    this.mistLayers = [];

    const c = this.condition;
    this.showSun = c === 'clear' || c === 'partly-cloudy';
    this.showClouds = c !== 'clear';
    this.darkClouds = ['rain','drizzle','thunderstorm','stormy','cloudy'].includes(c);

    if (c === 'rain' || c === 'drizzle') this.initRain(c === 'drizzle' ? 120 : 300);
    if (c === 'thunderstorm' || c === 'stormy') this.initRain(450, true);
    if (c === 'snow') this.initSnow();
    if (c === 'mist') this.initMist();
  }

  // ── RAIN INIT ──
  private initRain(count: number, heavy = false) {
    for (let i = 0; i < count; i++) {
      this.drops.push({
        x: Math.random() * this.W,
        y: Math.random() * this.H,
        len: heavy ? 18 + Math.random() * 20 : 12 + Math.random() * 14,
        speed: heavy ? 22 + Math.random() * 14 : 16 + Math.random() * 10,
        alpha: 0.35 + Math.random() * 0.45,
        width: heavy ? 1.2 + Math.random() * 0.8 : 0.8 + Math.random() * 0.6,
        // splash state
        splash: false, splashR: 0, splashX: 0, splashY: 0
      });
    }
  }

  // ── SNOW INIT ──
  private initSnow() {
    for (let i = 0; i < 180; i++) {
      this.flakes.push({
        x: Math.random() * this.W,
        y: Math.random() * this.H,
        r: 1.5 + Math.random() * 4,
        speed: 0.8 + Math.random() * 1.8,
        drift: (Math.random() - 0.5) * 0.8,
        alpha: 0.5 + Math.random() * 0.45,
        phase: Math.random() * Math.PI * 2
      });
    }
  }

  // ── MIST INIT ──
  private initMist() {
    for (let i = 0; i < 5; i++) {
      this.mistLayers.push({
        x: Math.random() * this.W,
        y: this.H * 0.3 + Math.random() * this.H * 0.5,
        w: 400 + Math.random() * 600,
        h: 80 + Math.random() * 120,
        speed: 0.15 + Math.random() * 0.25,
        alpha: 0.04 + Math.random() * 0.06
      });
    }
  }

  // ── MAIN LOOP ──
  private animate() {
    this.animId = requestAnimationFrame(() => this.animate());
    const ctx = this.ctx;
    ctx.clearRect(0, 0, this.W, this.H);

    const c = this.condition;
    if (c === 'rain' || c === 'drizzle') this.drawRain(ctx);
    if (c === 'thunderstorm' || c === 'stormy') { this.drawRain(ctx); this.handleLightning(ctx); }
    if (c === 'snow') this.drawSnow(ctx);
    if (c === 'mist') this.drawMist(ctx);
    if (c === 'clear') this.drawSunAtmosphere(ctx);
  }

  // ── DRAW RAIN ──
  private drawRain(ctx: CanvasRenderingContext2D) {
    const angle = 0.18; // slight slant (radians)
    const dx = Math.sin(angle);

    for (const d of this.drops) {
      // Move
      d.y += d.speed;
      d.x += d.speed * dx;

      // Splash when hitting bottom
      if (d.y > this.H) {
        d.y = -d.len;
        d.x = Math.random() * this.W;
        d.splash = false;
      }
      if (d.x > this.W) d.x -= this.W;

      // Draw raindrop — tapered line
      ctx.save();
      ctx.strokeStyle = `rgba(174,214,241,${d.alpha})`;
      ctx.lineWidth = d.width;
      ctx.lineCap = 'round';
      ctx.shadowColor = 'rgba(174,214,241,0.3)';
      ctx.shadowBlur = 2;
      ctx.beginPath();
      ctx.moveTo(d.x, d.y);
      ctx.lineTo(d.x - d.len * dx, d.y - d.len);
      ctx.stroke();
      ctx.restore();

      // Splash at floor
      if (d.y > this.H - 4 && !d.splash) {
        d.splash = true;
        d.splashX = d.x;
        d.splashY = this.H - 2;
        d.splashR = 0;
      }
      if (d.splash) {
        d.splashR += 1.2;
        ctx.save();
        ctx.strokeStyle = `rgba(174,214,241,${Math.max(0, 0.4 - d.splashR * 0.05)})`;
        ctx.lineWidth = 0.6;
        ctx.beginPath();
        ctx.ellipse(d.splashX, d.splashY, d.splashR * 1.8, d.splashR * 0.5, 0, 0, Math.PI * 2);
        ctx.stroke();
        ctx.restore();
        if (d.splashR > 8) d.splash = false;
      }
    }
  }

  // ── LIGHTNING ──
  private handleLightning(ctx: CanvasRenderingContext2D) {
    this.lightningTimer++;
    if (this.lightningTimer > 90 + Math.random() * 150) {
      this.lightningTimer = 0;
      this.drawLightningBolt(ctx);
      this.zone.run(() => {
        this.lightningFlash = true;
        setTimeout(() => this.lightningFlash = false, 100);
      });
    }
  }

  private drawLightningBolt(ctx: CanvasRenderingContext2D) {
    const startX = this.W * 0.2 + Math.random() * this.W * 0.6;
    let x = startX, y = 0;
    const segments: {x: number, y: number}[] = [{x, y}];
    while (y < this.H * 0.7) {
      x += (Math.random() - 0.5) * 70;
      y += 40 + Math.random() * 60;
      segments.push({x, y});
    }

    // Glow pass
    ctx.save();
    ctx.strokeStyle = 'rgba(180,200,255,0.25)';
    ctx.lineWidth = 8;
    ctx.shadowColor = '#a5b4fc';
    ctx.shadowBlur = 30;
    ctx.beginPath();
    ctx.moveTo(segments[0].x, segments[0].y);
    segments.slice(1).forEach(s => ctx.lineTo(s.x, s.y));
    ctx.stroke();

    // Core pass
    ctx.strokeStyle = 'rgba(220,230,255,0.95)';
    ctx.lineWidth = 1.5;
    ctx.shadowBlur = 10;
    ctx.beginPath();
    ctx.moveTo(segments[0].x, segments[0].y);
    segments.slice(1).forEach(s => ctx.lineTo(s.x, s.y));
    ctx.stroke();
    ctx.restore();
  }

  // ── DRAW SNOW ──
  private drawSnow(ctx: CanvasRenderingContext2D) {
    const t = Date.now() / 1000;
    for (const f of this.flakes) {
      f.y += f.speed;
      f.x += f.drift + Math.sin(t * 0.5 + f.phase) * 0.4;
      if (f.y > this.H + f.r) { f.y = -f.r; f.x = Math.random() * this.W; }
      if (f.x > this.W + f.r) f.x = -f.r;
      if (f.x < -f.r) f.x = this.W + f.r;

      ctx.save();
      const grd = ctx.createRadialGradient(f.x, f.y, 0, f.x, f.y, f.r);
      grd.addColorStop(0, `rgba(240,248,255,${f.alpha})`);
      grd.addColorStop(0.6, `rgba(210,235,255,${f.alpha * 0.7})`);
      grd.addColorStop(1, 'rgba(210,235,255,0)');
      ctx.fillStyle = grd;
      ctx.beginPath();
      ctx.arc(f.x, f.y, f.r * 1.2, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    }
  }

  // ── DRAW MIST ──
  private drawMist(ctx: CanvasRenderingContext2D) {
    for (const m of this.mistLayers) {
      m.x += m.speed;
      if (m.x > this.W + m.w) m.x = -m.w;

      const grd = ctx.createRadialGradient(
        m.x + m.w / 2, m.y + m.h / 2, 0,
        m.x + m.w / 2, m.y + m.h / 2,
        Math.max(m.w, m.h) * 0.6
      );
      grd.addColorStop(0, `rgba(180,200,220,${m.alpha})`);
      grd.addColorStop(1, 'rgba(180,200,220,0)');
      ctx.fillStyle = grd;
      ctx.beginPath();
      ctx.ellipse(m.x + m.w / 2, m.y + m.h / 2, m.w * 0.6, m.h * 0.5, 0, 0, Math.PI * 2);
      ctx.fill();
    }
  }

  // ── SUN ATMOSPHERE (subtle light rays on canvas) ──
  private drawSunAtmosphere(ctx: CanvasRenderingContext2D) {
    const t = Date.now() / 4000;
    const cx = this.W * 0.83;
    const cy = this.H * 0.12;

    // Soft halo
    const halo = ctx.createRadialGradient(cx, cy, 0, cx, cy, 300);
    halo.addColorStop(0, 'rgba(255,220,80,0.12)');
    halo.addColorStop(0.4, 'rgba(255,170,40,0.06)');
    halo.addColorStop(1, 'rgba(0,0,0,0)');
    ctx.fillStyle = halo;
    ctx.beginPath();
    ctx.arc(cx, cy, 300, 0, Math.PI * 2);
    ctx.fill();

    // Rotating crepuscular rays
    for (let i = 0; i < 8; i++) {
      const angle = t + (i * Math.PI * 2) / 8;
      const len = 250 + Math.sin(t * 2 + i) * 60;
      const alpha = 0.04 + Math.sin(t + i * 1.3) * 0.02;
      ctx.save();
      ctx.strokeStyle = `rgba(255,220,100,${Math.max(0, alpha)})`;
      ctx.lineWidth = 18 + i * 3;
      ctx.lineCap = 'round';
      ctx.beginPath();
      ctx.moveTo(cx, cy);
      ctx.lineTo(cx + Math.cos(angle) * len, cy + Math.sin(angle) * len);
      ctx.stroke();
      ctx.restore();
    }
  }
}

interface Drop {
  x: number; y: number; len: number; speed: number;
  alpha: number; width: number;
  splash: boolean; splashR: number; splashX: number; splashY: number;
}
interface Flake {
  x: number; y: number; r: number; speed: number;
  drift: number; alpha: number; phase: number;
}
interface MistLayer {
  x: number; y: number; w: number; h: number;
  speed: number; alpha: number;
}